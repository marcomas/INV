<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="UploadFile.aspx.vb" Inherits="INV.UploadFile" %>
...
<head runat="server">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#selFiles').change(function() {
                var filename = $('#selFiles>option:selected').text();
                $('#viewImg').attr('src', '../imgINV/' + '<%=Folder %>' + '/' + '<%=IdRecord %>' + '/' + filename);

                $.ajax({
                    type: "POST",
                    url: "../WS/WSSessionVar.aspx",
                    data: { name: 'filename', fun: 'set', value: filename },
                    async: false
                }).done(function(data) {
                });
            });
...
<body>
    <div id="divContFiles" runat="server" style="float:right;">
        <div id="div1" style="float:right; position:relative;">
            <select id="selFiles" style="top:-7px; position:relative;" runat="server"></select>
        </div>
    </div>
    <form id="Form1" runat="server">        
        <div class="Multifile-vk-panel">  
            ...
            <img id="viewImg" width="100px" height="100px" /><br />
        </div>
    </form>
</body>
